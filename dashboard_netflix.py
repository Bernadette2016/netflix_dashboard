...
# Slider for selecting release year range
...
tabs = st.tabs([
    ...
    "Rating by Genre",
    "Genres: Movies vs TV Shows"
])

with tabs[0]:
    type_counts = filtered_df['type'].value_counts()
    fig = px.pie(names=type_counts.index, values=type_counts.values, title="Distribution of Content Types")
    st.plotly_chart(fig)
...
with tabs[3]:
    ...
    fig = px.bar(x=top_genres.index, y=top_genres.values, labels={'x': 'Genre', 'y': 'Number of Titles'}, title='Top 10 Genres on Netflix')
...
# Footer
st.sidebar.markdown("---")
st.sidebar.markdown("Use the filters to explore the Netflix dataset by content type and year.")
